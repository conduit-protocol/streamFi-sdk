...
All methods throw `ConduitError` on failure:

```typescript
import { ConduitError, ErrorCode } from '@conduit-protocol/sdk';
...
try {
  ...
  if (err instanceof ConduitError) {
    switch (err.code) {
      case ErrorCode.NothingToWithdraw:
...
│   ├── errors.ts            # ConduitError + ErrorCode
