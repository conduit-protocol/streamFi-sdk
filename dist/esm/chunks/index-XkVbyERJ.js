import { Networks, xdr, SorobanRpc, Contract, TransactionBuilder, BASE_FEE, nativeToScVal, Address } from '@stellar/stellar-sdk';
...
class FactoryModule {
    ...
    constructor(config) {
        ...
        if (!config.factoryAddress) {
            throw new Error(`ConduitConfig.factoryAddress is required (no default DripFactory is known for network "${config.network}").`);
        }
    ...
    async streamAddress(streamId) {
        ...
        const tx = await buildContractCallTx(this.rpcUrl, this.passphrase, this.callerAddr, this.factoryId, 'stream_address', [nativeToScVal(id, { type: 'u64' })]);
        const val = await simulateReadOnly(this.rpcUrl, this.passphrase, tx);
...
})(FactoryErrorCode || (FactoryErrorCode = {}));
...
})(GovernorErrorCode || (GovernorErrorCode = {}));
...
class StreamsModule {
    ...
    async subscribeAsync(streamId, handlers) {
        ...
        const { subscribeToStream } = await import('./events-BK26HC5R.js');
    ...
    async _resolveAddr(id) {
        ...
        if (!addr)
            throw new ConduitError('stream', StreamErrorCode.StreamNotFound, `Stream ${id} not found`);
        return addr;
    }
...
class GovernorModule {
    ...
    async getConfig() {
        if (!this.governorId) {
            throw new Error(`ConduitConfig.governorAddress is required (no default DripGovernor is known for network "${this.network}").`);
        }
...
export { ConduitClient as C, FactoryErrorCode as F, GovernorErrorCode as G, StreamErrorCode as S, scValToU64 as a, ConduitError as b, scValToI128 as s };
//# sourceMappingURL=index-XkVbyERJ.js.map
